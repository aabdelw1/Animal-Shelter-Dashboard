...
const AddAnimalModal = (props) => {
  ...
  const [animalInfoTab, showAnimalInfoTab] = useState(true)
  const [surrenderInfoTab, showSurrenderInoTab] = useState(false)
  const [vaccineInfoTab, showVaccineInfoTab] = useState(false)
  ...
  function whenClicked (state) {
    if(state === 0){
      showAnimalInfoTab(true)
      showSurrenderInoTab(false)
      showVaccineInfoTab(false)
    }
    else if(state === 1) {
      showAnimalInfoTab(false)
      showSurrenderInoTab(true)
      showVaccineInfoTab(false)
    }
    else if(state === 2) {
      showAnimalInfoTab(false)
      showSurrenderInoTab(false)
      showVaccineInfoTab(true)
    }
  ...
  return (
    ...
    <Pane display="flex" flexDirection="column">
      <Component
        initialState={{
          selectedIndex: 0,
          tabs: ['Animal Info', 'Surrender Info', 'Vaccine Info']
        }}
      >
        {({ state, setState }) => (
          <Pane height={300}>
            ...
            <Pane padding={10} background="tint1" flex="1">
              {state.tabs.map((tab, index) => (
                <Pane
                  key={tab}
                  id={`panel-${tab}`}
